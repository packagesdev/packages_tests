...
test_displayed_name="uds > raw package > additional resources > custom folder - dynamic value"
...
projectpath = os.path.join(dirname, 'test_uds_rawpackage_additional_resources_custom_folder.pkgproj')
...
process = subprocess.Popen(['/usr/bin/ditto', '-x',os.path.join(dirname, 'extracted','Scripts'), os.path.join(dirname, 'extracted','Scripts_unarchived')],
                           stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE)
stdout, stderr = process.communicate()


if (os.path.exists(os.path.join(dirname, 'extracted','Scripts_unarchived',expected_custom_folder_name)) == True ):
