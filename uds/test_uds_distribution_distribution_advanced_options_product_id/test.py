...
# Check that the name of the distribution defined as the PRODUCT_ID user defined setting is correctly converted during the build process

test_displayed_name="uds > distribution > advanced option > product id - dynamic value"

# Given

user_defined_product_id='inc.acme.myapp'
...
process = subprocess.Popen(['/usr/local/bin/packagesbuild', '--project', projectpath , 'PRODUCT_ID={}'.format(user_defined_product_id)],
                     stdout=subprocess.PIPE, 
                     stderr=subprocess.PIPE)
stdout, stderr = process.communicate()
...
expected_distribution_product_id='inc.acme.myapp'
